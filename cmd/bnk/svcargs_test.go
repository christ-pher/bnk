...
func TestServiceArgsIncludesKeyOnFirstInstall(t *testing.T) {
	...
	want := []string{"run", "--state-dir", `C:\ProgramData\bnk`, "--server", "https://vps:8443", "--key", "bnkkey:secret:fp"}
...
func TestServiceArgsOmitsEmptyKey(t *testing.T) {
	...
	want := []string{"run", "--state-dir", `C:\ProgramData\bnk`, "--server", "https://vps:8443"}
	if !slices.Equal(got, want) {
		t.Errorf("serviceArgs = %v, want %v", got, want)
	}
}
...
func TestServiceArgsKeepsOperatorWithoutKey(t *testing.T) {
	...
	want := []string{"run", "--state-dir", `C:\ProgramData\bnk`, "--server", "https://vps:8443", "--operator", sid}
	if !slices.Equal(got, want) {
		t.Errorf("serviceArgs = %v, want %v", got, want)
	}
}
...
// A machine installed with no server at all must still get a runnable
// service: it idles until someone signs in from the tray.
func TestServiceArgsWithoutAServer(t *testing.T) {
	got := serviceArgs("", "", `C:\ProgramData\bnk`, "S-1-5-21-1-2-3-1001")
	if slices.Contains(got, "--server") {
		t.Errorf("serviceArgs = %v, want no --server when none is known", got)
	}
	if got[0] != "run" {
		t.Errorf("serviceArgs = %v, want it to start the daemon", got)
	}
}
